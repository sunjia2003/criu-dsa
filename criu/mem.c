...
struct dsa_parent_coverage_entry {
	u64 start;
	u64 end;
};

struct dsa_parent_coverage {
	struct dsa_parent_coverage_entry *entries;
	size_t nr;
	size_t cap;
	bool ready;
	bool has_parent;
...
static struct dsa_parent_coverage dsa_parent_coverage_prev;

static void dsa_parent_coverage_reset(struct dsa_parent_coverage *cov)
{
	if (!cov)
		return;
	xfree(cov->entries);
	...
	cov->cap = 0;
	cov->ready = false;
	cov->has_parent = false;
}

void dsa_parent_coverage_cleanup_after_dump(void)
{
	dsa_parent_coverage_reset(&dsa_parent_coverage_prev);
}

static int dsa_parent_coverage_reserve(struct dsa_parent_coverage *cov, size_t need)
{
	struct dsa_parent_coverage_entry *new_entries;
	size_t new_cap;

	if (need <= cov->cap)
		return 0;

	new_cap = cov->cap ? cov->cap * 2 : 1024;
	while (new_cap < need)
		new_cap *= 2;

	new_entries = xrealloc(cov->entries, new_cap * sizeof(*cov->entries));
	if (!new_entries)
		return -1;
	cov->entries = new_entries;
	cov->cap = new_cap;
	return 0;
}

static int dsa_parent_coverage_add_range(struct dsa_parent_coverage *cov, u64 start, u64 len)
{
	u64 end;
	...
	if (cov->nr > 0 && start <= cov->entries[cov->nr - 1].end) {
		if (end > cov->entries[cov->nr - 1].end)
			cov->entries[cov->nr - 1].end = end;
		return 0;
	}

	if (dsa_parent_coverage_reserve(cov, cov->nr + 1))
		return -1;
	cov->entries[cov->nr].start = start;
	cov->entries[cov->nr].end = end;
	cov->nr++;
	return 0;
}

static int dsa_parent_coverage_cmp(const void *a, const void *b)
{
	const struct dsa_parent_coverage_entry *ea = a;
	const struct dsa_parent_coverage_entry *eb = b;
...
static void dsa_parent_coverage_sort_merge(struct dsa_parent_coverage *cov)
{
	size_t i;
	size_t out = 0;
	...
	qsort(cov->entries, cov->nr, sizeof(*cov->entries), dsa_parent_coverage_cmp);
	for (i = 0; i < cov->nr; i++) {
		if (cov->entries[i].start >= cov->entries[i].end)
			continue;
		if (out && cov->entries[i].start <= cov->entries[out - 1].end) {
			if (cov->entries[i].end > cov->entries[out - 1].end)
				cov->entries[out - 1].end = cov->entries[i].end;
		} else {
			cov->entries[out++] = cov->entries[i];
		}
	}
	cov->nr = out;
}

static bool dsa_parent_coverage_contains_range(const struct dsa_parent_coverage *cov, u64 start, u64 len)
{
	size_t lo = 0, hi;
	u64 end;

	if (!cov || !cov->ready || !len)
		return false;
	end = start + len;
	if (end <= start)
		return false;
	...
	while (lo < hi) {
		...
		if (cov->entries[mid].end <= start)
			lo = mid + 1;
		else
			hi = mid;
	}

	return lo < cov->nr && cov->entries[lo].start <= start && cov->entries[lo].end >= end;
}

static void dsa_init_compat_pagemap_entry(PagemapEntry *pe)
{
...
int dsa_parent_coverage_prepare_before_freeze(unsigned long img_id)
{
	int pfd = -1;
	int ret = 0;
	u32 pages_id;
	...
	dsa_parent_coverage_cleanup_after_dump();
	...
	pmi = open_image_at(pfd, CR_FD_PAGEMAP, O_RSTR, img_id);
	if (!pmi) {
		ret = -1;
		goto out;
	}
	...
	/* Consume the pagemap header and associated pages image id before reading entries. */
	pi = open_pages_image_at(pfd, O_RSTR, pmi, &pages_id);
	if (!pi) {
		ret = -1;
		goto out;
	}
	...
	while (1) {
		ret = pb_read_one_eof(pmi, &pe, PB_PAGEMAP);
		if (ret < 0)
			goto out;
		if (ret == 0)
			break;

		dsa_init_compat_pagemap_entry(pe);
		if (pe->flags & (PE_PRESENT | PE_PARENT | PE_LAZY)) {
			if (dsa_parent_coverage_add_range(&dsa_parent_coverage_prev,
							pe->vaddr, pagemap_len(pe))) {
	...
	dsa_parent_coverage_sort_merge(&dsa_parent_coverage_prev);
	dsa_parent_coverage_prev.ready = true;
	dsa_parent_coverage_prev.has_parent = true;
	pr_info("DSA_PARENT_COVERAGE: loaded direct parent entries=%zu\n",
		dsa_parent_coverage_prev.nr);
	ret = 0;
out:
	if (pe)
		pagemap_entry__free_unpacked(pe, NULL);
...
static int dsa_stream_layout_init(struct dsa_desc_scan_ctx *sc)
{
	...
	sc->fg_result_meta_base = fg_meta_base;
	sc->fg_result_meta_head = fg_meta_base;
	sc->fg_result_meta_limit = fg_meta_limit;
	...
	if (raw_fg_capture) {
		...
		sc->stream_hdr->fg_result_data_base = fg_data_base;
		sc->stream_hdr->fg_result_data_limit =
			sc->dsa_ctx->shared_buf_size;
		dsa_shared_store_u32(&sc->stream_hdr->fg_result_data_head,
				     fg_data_base);
	}
...
static int start_deferred_dsa_replay(struct mem_dump_async *async)
{
	...
	if (!async->dsa_ctx_deferred->replay.ready)
		return 0;
	...
	for (vaddr = *pvaddr; vaddr < vma->e->end; vaddr += PAGE_SIZE, nr_scanned++) {
...

		{
			...
			if (has_parent) {
				if (sc->scan_profile)
					t0 = dsa_wall_now_us();
				parent_has_page = dsa_parent_coverage_contains_range(&dsa_parent_coverage_prev,
										     vaddr, PAGE_SIZE);
				if (sc->scan_profile) {
					t1 = dsa_wall_now_us();
			...
			in_parent = has_parent && page_in_parent(page_info.softdirty) &&
				parent_has_page;
					...
					struct parasite_ctl *ctl, struct memdump_timeline *timeline)
{
	...
	args->off = 0;
	if (dsa_desc_scan_active)
		has_parent = dsa_parent_coverage_prev.ready;
	else
	...
	memdump_timeline_switch(timeline, MEMDUMP_PHASE_SCAN_BUILD);
	if (dsa_desc_scan_active)
		scan_build_start_us = dsa_wall_now_us();
	...
	if (dsa_desc_scan_active) {
		...
		scan_build_us = dsa_wall_delta_us(scan_build_start_us, dsa_wall_now_us());
		dsa_sc.scan_wall_us = scan_build_us;
	}

	if (dsa_desc_scan_active && dsa_desc_scan_ready) {
		memdump_timeline_switch(timeline, MEMDUMP_PHASE_DSA_RPC);
		dsa_rpc_start_us = dsa_wall_now_us();
#ifdef CRIU_DSA_ENABLE_LEGACY_SINGLE_RPC
		if (dsa_sc.legacy_single_rpc) {
			detail_t0 = dsa_wall_now_us();
			ret = dsa_legacy_single_rpc_finish(&dsa_sc);
			dsa_stream_finish_us = dsa_wall_delta_us(detail_t0, dsa_wall_now_us());
		} else
#endif
		{
			detail_t0 = dsa_wall_now_us();
...
int prepare_mappings(struct pstree_item *t)
{
	...
	struct page_read pr;
	uint64_t total_start_us = restore_now_us();
	uint64_t stage_start_us;
	uint64_t reserve_us = 0;
	uint64_t open_page_read_us = 0;
	uint64_t maybe_disable_thp_us = 0;
	uint64_t premap_us = 0;
	uint64_t restore_content_us = 0;

	void *old_premmapped_addr = NULL;
	unsigned long old_premmapped_len = 0;
	...
	open_page_read_us = restore_now_us() - stage_start_us;
	if (ret <= 0) {
		ret = -1;
		goto out;
	}
	...
	maybe_disable_thp_us = restore_now_us() - stage_start_us;

	pr.advance(&pr); /* shift to the 1st iovec */

	stage_start_us = restore_now_us();
	ret = premap_priv_vmas(t, vmas, &addr, &pr);
	premap_us = restore_now_us() - stage_start_us;
	if (ret < 0)
		goto out;
	...
	restore_content_us = restore_now_us() - stage_start_us;
	if (ret < 0)
		goto out;
...
out:
	pr_info("RESTORE_MAPPING_TIMING pid=%d ret=%d rst_priv_size=%lu reserve_us=%" PRIu64 " open_page_read_us=%" PRIu64 " maybe_disable_thp_us=%" PRIu64 " premap_us=%" PRIu64 " restore_content_us=%" PRIu64 " total_us=%" PRIu64 "\n",
		vpid(t), ret, vmas ? vmas->rst_priv_size : 0,
		reserve_us, open_page_read_us, maybe_disable_thp_us,
		premap_us, restore_content_us, restore_now_us() - total_start_us);
	return ret;
}
